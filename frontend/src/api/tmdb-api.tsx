...
// add your access token here as Bearer <Your access token> directly or import it from .env
axiosClient.interceptors.request.use((config) => {
  config.headers[
    "Authorization"
  ] = `Bearer ${import.meta.env.REACT_APP_TMDB_ACCESS_TOKEN}`
  return config
})
...
export const getTopRated = async (
  ...
  try {
    const { data } = await axiosClient.get<
      any,
      AxiosResponse<{ results: unknown[], total_pages: number }>
    >(`/${mediaType}/top_rated`, {
      params: {
        page,
      },
